...
%% Dritte Variante
%% den maximalen Wert im linken Teilbaum (immer dem rechten Nachfolger
%% entlang), oder
loeschenHelper(Tree, Key, File, LeftChildKey, RightChildKey) when (LeftChildKey /= nil) and (RightChildKey /= nil) ->
  ...
  [ Part1, Part2 | NodeList ]= Tree,
  [PrefixX, KeyX, [LeftChildX, RightChildX], HeightX, PredecessorX] = KeyNode,
  ...
  true ->
    ...
    % Vater von MaxKey soll auf nichts mehr zeigen
    ...
    io:nl(), io:fwrite("ModifyTree2: "), io:write(ModifyTree2), io:nl(),
    ModifyTree2

    %io:nl(), io:fwrite("Result: "), io:write(ModifyTree), io:nl(),
    %ModifyTree2 = ModifyTree
...
loeschenHelper(Tree, Key, File, LeftChildKey, RightChildKey) ->
...
doppelLinksrotation(Tree, Key, File) ->
  ...
  io:fwrite("Jetzt sollte die links um den Key 356 kommen " ), io:nl(),
